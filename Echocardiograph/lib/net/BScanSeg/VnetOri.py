...
class UpTransition(nn.Module):
    ...
    def forward(self, x, skipx):
        out = self.do1(x)   # 2C channels
        skipxdo = self.do2(skipx)  # 1C channel from encoder
...
class VNet(nn.Module):
    ...
    def __init__(self, elu=True, nll=False, out_channel=2):
        ...
        self.in_tr = InputTransition(16, elu)  # size = input_X
        self.down_tr32 = DownTransition(16, 1, elu)  # 1 layer in 16 out 32 size=input_X/2
        self.down_tr64 = DownTransition(32, 2, elu)  # 2 layers in 32 out 64 size=input_X/4
        self.down_tr128 = DownTransition(64, 3, elu, dropout=True)  # 3 layers in 64 out 128 size=input_X/8
        self.down_tr256 = DownTransition(128, 2, elu, dropout=True)  # 2 layers in 128 out 256 size=input_X/16
        self.up_tr256 = UpTransition(256, 256, 2, elu, dropout=True)  # 2 layers in 256 out 256 size=input_X/2
        self.up_tr128 = UpTransition(256, 128, 2, elu, dropout=True)
        self.up_tr64 = UpTransition(128, 64, 1, elu)
        self.up_tr32 = UpTransition(64, 32, 1, elu)
        self.out_tr = OutputTransition(32, elu, nll, out_channel)  # out channel = ? num_channel
        self.out_map = OutputTransitionMap(32, elu, nll, out_channel)  # out channel = ?  num_channel
    ...
    def forward(self, x):
        out16 = self.in_tr(x)   # [1, 16, x, y, z]
        out32 = self.down_tr32(out16)  # [1, 32, x, y, z]
        out64 = self.down_tr64(out32)  # [1, 64, x, y, z]
        out128 = self.down_tr128(out64)  # [1, 128, x, y, z]
        out256 = self.down_tr256(out128)  # [1, 256, x, y, z]
        # torch.save(out16, 'out16E.pt')
        # torch.save(out32, 'out32E.pt')
        # torch.save(out64, 'out64E.pt')
        # torch.save(out128, 'out128E.pt')
        # torch.save(out256, 'out256E.pt')
        out = self.up_tr256(out256, out128)  # [1 ,256, 112, 112, 112]  [256+256] -- [256]
        # torch.save(out, 'out256D.pt')
        out = self.up_tr128(out, out64)  # [1 ,128, 112, 112, 112]
        # torch.save(out, 'out128D.pt')
        out = self.up_tr64(out, out32)  # [1 ,64, 112, 112, 112]
        # torch.save(out, 'out64D.pt')
        out = self.up_tr32(out, out16)  # [1 ,32, 112, 112, 112] (B,C,W H Z) b=1 @ one card
        # torch.save(out, 'out32D.pt')

        out02 = self.out_map(out)  # [B, n, 112, 112, 112]   warning !!!
...
if __name__ == '__main__':
    ...
    summary(test_net, (1, 112, 112, 160), batch_size=1)  # channel/ depth / width / height
